...
export default async function ShoppingListPage() {
    ...
    return (
        <div className="min-h-screen bg-cream-bg dark:bg-dark-bg flex flex-col pb-24">
            {/* Header */}
            <header className="flex items-center justify-between px-6 py-4 sticky top-0 z-50 bg-white/95 dark:bg-dark-bg/95 backdrop-blur-sm border-b border-cream-surface dark:border-dark-border shadow-sm">
                <Link href="/dashboard" className="text-[var(--color-primary-dark)] dark:text-gray-100 hover:opacity-70 transition-opacity">
                    <ArrowLeft className="w-6 h-6" />
                </Link>
                <h1 className="font-serif text-xl font-bold tracking-tight text-center text-[var(--color-primary-dark)] dark:text-gray-100">
                    Mi Lista de Compras
                </h1>
            ...
            <main className="flex-1 w-full max-w-md mx-auto px-4 pt-6">
                <div className="bg-white dark:bg-dark-surface rounded-2xl shadow-sm p-5 border border-cream-surface dark:border-dark-border">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 pb-3 border-b border-gray-100 dark:border-gray-800 flex items-center gap-2">
                        ...
                        <ul className="space-y-4">
                            {ingredientes.map((item, idx) => (
                                <li key={idx} className="flex items-center gap-3">
                                    <div className="w-5 h-5 rounded border-2 border-gray-300 dark:border-gray-600 flex-shrink-0 cursor-pointer hover:border-terracotta transition-colors"></div>
                                    <div className="flex-1 text-[var(--color-foreground)] text-base font-medium">
                                        {item.nombre}
                                    </div>
                                    <div className="text-sm text-gray-500 font-mono bg-gray-50 dark:bg-black/20 px-2 py-1 rounded">
                ...
                {/* Info Box */}
                <div className="mt-6 bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-900/40">
