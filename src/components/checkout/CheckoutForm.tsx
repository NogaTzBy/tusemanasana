...
export function CheckoutForm({ emailPrellenado }: CheckoutFormProps) {
    ...
    return (
        <main className="min-h-screen bg-[var(--color-background)]">
            ...
            <section className="max-w-4xl mx-auto pt-4 pb-8 px-4 md:px-6 grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12">
                {/* Lado izquierdo: Propuesta de valor */}
                <div>
                    <h1 className="text-xl md:text-2xl font-serif text-[var(--color-primary-dark)] mb-1 md:mb-2">
                        Un solo pago, tuya para siempre
                    </h1>
                    <p className="text-sm md:text-base text-[var(--color-foreground)]/70 mb-4 md:mb-8">
                        Sin suscripciones. Sin sorpresas. Pagás una vez y accedés a todos los menús y actualizaciones para siempre.
                    </p>

                    <div className="space-y-2 md:space-y-4 mb-4 md:mb-8 text-sm md:text-base">
                        <div className="flex items-start gap-2 md:gap-3">
                            <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5 text-[var(--color-primary)] flex-shrink-0 mt-0.5" />
                            <span className="text-[var(--color-foreground)]/80 leading-snug">Menús semanales personalizados con IA según tus gustos</span>
                        </div>
                        <div className="flex items-start gap-2 md:gap-3">
                            <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5 text-[var(--color-primary)] flex-shrink-0 mt-0.5" />
                            <span className="text-[var(--color-foreground)]/80 leading-snug">Lista de compras exacta, sin desperdiciar nada</span>
                        </div>
                        <div className="flex items-start gap-2 md:gap-3">
                            <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5 text-[var(--color-primary)] flex-shrink-0 mt-0.5" />
                            <span className="text-[var(--color-foreground)]/80 leading-snug">Cambiá cualquier receta con un clic</span>
                        </div>
                        <div className="flex items-start gap-2 md:gap-3">
                            <Infinity className="w-4 h-4 md:w-5 md:h-5 text-[var(--color-primary)] flex-shrink-0 mt-0.5" />
                            <span className="text-[var(--color-foreground)]/80 leading-snug">Actualizaciones y nuevas recetas incluidas para siempre</span>
                        </div>
                    </div>

                    <div className="bg-[var(--color-primary)]/10 text-[var(--color-primary-dark)] p-3 md:p-4 rounded-xl text-xs md:text-sm flex items-start">
                        <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5 flex-shrink-0 mr-2 md:mr-3 mt-0.5" />
                        <p>
                ...
                <div className="bg-white p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 relative">
                    {isSubmitting && (
                    ...
                    <h2 className="text-base md:text-lg font-semibold mb-3 md:mb-6 flex items-center">
                        <Lock className="w-4 h-4 mr-2 text-gray-400" /> Checkout seguro
                    </h2>

                    <p className="text-xs md:text-sm text-[var(--color-foreground)]/70 mb-4 md:mb-6">
                        Serás redirigida a la pasarela de pago oficial. Aceptamos Mercado Pago y tarjetas.
                    </p>

                    <form className="space-y-3 md:space-y-4" onSubmit={handleCheckout}>
                        <div>
                        ...
                        <div className="pt-3 md:pt-4 border-t border-gray-100 mt-4 md:mt-6">
                            <div className="flex justify-between items-center mb-1 text-[var(--color-foreground)]">
                                ...
                                <span className="font-bold text-xl md:text-2xl">$19.900 ARS</span>
                            </div>
                            <p className="text-[10px] md:text-xs text-[var(--color-foreground)]/50 mb-3 md:mb-4">Pago único · Acceso de por vida</p>

                            <button
                                type="submit"
                                className="w-full py-3 md:py-4 text-white bg-black rounded-lg md:rounded-xl text-sm md:text-base font-medium shadow hover:bg-gray-800 transition-colors disabled:opacity-50"
